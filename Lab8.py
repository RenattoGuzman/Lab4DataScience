# para ejecutar el programa corren en un entorno python "streamlit run Lab8.py"
...
sns.barplot(x=brand_counts.index[:10], y=brand_counts.values[:10], palette=colors)
...
plt.figure(figsize=(10, 6))
sns.histplot(df['reviews.rating'], bins=5, kde=False, color=colors[1])
...
plt.figure(figsize=(10, 6))
sns.scatterplot(x='dateAdded', y='reviews.rating', data=df, color=colors[2])
...
plt.figure(figsize=(10, 6))
sns.barplot(x=helpful_counts.index[:10], y=helpful_counts.values[:10], palette=colors)
